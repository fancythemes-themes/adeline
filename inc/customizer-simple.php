 ...
 * Return this array in a function and add_filter to 'adeline_customizer_wrapper_settings' hook
 * Create customizer settings and controls based the arguments
 * with filter 'adeline_customizer_wrapper_settings'
 */
function adeline_customizer_wrapper_settings( $wp_customize ) {

	do_action('adeline_before_customize_wrapper', $wp_customize );

	$settings = array();
	$customize_admin_js_var = array();

	$settings = apply_filters( 'adeline_customizer_wrapper_settings', $settings );
	$i = 1;
	...
	do_action('adeline_after_customize_wrapper', $wp_customize );
}
add_action( 'customize_register', 'adeline_customizer_wrapper_settings', 100, 1 );


/** 
 * Apply the CSS set by the customizer framework based on arguments 
 */
function adeline_apply_customizer_css(){

	$settings = array();
	$settings = apply_filters( 'adeline_apply_customizer_css', $settings );
	...
	if ( is_customize_preview() ) {
		wp_enqueue_style( 'adeline-preview-style', get_template_directory_uri() . '/css/customize-preview.css', array( 'adeline-style' ), '20160816' );
		wp_add_inline_style( 'adeline-preview-style', $css );
		//wp_add_inline_style( 'adeline-style', esc_attr($css) );
	} else {
		wp_add_inline_style( 'adeline-style', $css );
	}
	wp_localize_script('adeline-customize-preview', '_customizerCSS', $customize_js_var );

}
add_action('wp_enqueue_scripts', 'adeline_apply_customizer_css', 11 );
...
function adeline_customize_preview_js() {
	wp_enqueue_script( 'adeline-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20130508', true );
}
 ...
 */
function adeline_customize_admin_js() {
	wp_enqueue_script( 'adeline-customizer-admin', get_template_directory_uri() . '/js/customize-control.js', array( ), '20141114', true );
}
add_action( 'customize_controls_enqueue_scripts', 'adeline_customize_admin_js' );