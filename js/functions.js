...
( function( $ ) {
	...
	( function() {
		...
		menuToggle.on( 'click.adeline', function() {
	...
	( function() {
		...
		function toggleFocusClassTouchScreen() {
			if ( window.innerWidth >= 910 ) {
				$( document.body ).on( 'touchstart.adeline', function( e ) {
					if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
						$( '.main-navigation li' ).removeClass( 'focus' );
					}
				} );
				siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.adeline', function( e ) {
				...
				siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.adeline' );
			}
		}

		if ( 'ontouchstart' in window ) {
			$( window ).on( 'resize.adeline', toggleFocusClassTouchScreen );
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find( 'a' ).on( 'focus.adeline blur.adeline', function() {
		...
		$('.posts-slider').each( function(){
			...
			slider.flexslider( {
				...
				init: function( slider ) {
					...
					slider.doMath();
					$(window).on('resize.adeline', function() {
						sliderWidthInit(slider);
						slider.doMath();
					});
				}
 		...
 		$('.load-more a').on('click.adeline', function (e) {
			e.preventDefault();
